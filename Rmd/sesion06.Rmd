...
+ Aunque `table` está bien, hay varias librerías de R que extienden mucho su funcionalidad y la información. Por ejemplo, prueba a instalar la librería `gmodels` y luego ejecuta estos comandos para ver la cantidad de información que puedes obtener :\small
    ```{r eval = FALSE, purl=FALSE, comment=NULL, echo = -(1:2)}
    # Usando CrossTable
...
+ Después de leer los datos a un data.frame de R que vamos a llamar CHDdata, lo exploramos:\scriptsize    
    ```{r echo= -(1:3)}
    # Modelos lineales generalizados (glm). Regresión Logística. 
...
+ Recuerda que siempre conviene empezar explorando los datos. En este caso usamos `summary` (y comprobamos que no hay datos ausentes):\scriptsize
    ```{r}
    summary(CHDdata)
    ```
  \normalsize 
...
+ Ya podemos interpretar $\beta_1$: al aumentar $x$ en una unidad los log-odds $\beta_0+\beta_1 x$ aumentan en $\beta$. Después podemos traducir los odds en términos de probabilidades, como hemos visto. 
...
# Complementos de R. Más herramientas del tidyverse.
